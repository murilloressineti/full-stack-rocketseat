...
function sortTicketsByStatusAndUpdateDate(tickets: Ticket[]) {
...
function formatTicketListItem(ticket: Ticket): TicketListItem {
  return {
    id: ticket.id,
...
export default function AdminTicketsList() {
    ...
    async function loadTickets() {
      try {
        ...
        const formattedTickets =
          sortTicketsByStatusAndUpdateDate(data).map(formatTicketListItem);
  ...
  function handleOpenDetails(ticket: TicketListItem) {
  ...
  return (
    <div className="flex flex-col gap-4 md:gap-6">
      ...
      <div className="overflow-hidden rounded-xl border border-gray-200">
        <div className="grid grid-cols-[1.2fr_2fr_0.8fr] gap-2 border-b border-gray-200 px-4 py-4 md:grid-cols-[1.1fr_0.6fr_2.4fr_1fr_1.5fr_1.5fr_1.4fr_0.3fr]">
        ...
        {tickets.length > 0 ? (
          tickets.map((ticket) => (
            <TicketRow
              ...
              status={ticket.status}
              onDetails={() => handleOpenDetails(ticket)}
            />
          ))
        ) : (
